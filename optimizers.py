...
class AdamCon(Optimizer):
    ...
    def step(self, real_data, labels, N):
        ...
        fake_data = self.G(noise(N, 100).to(self.G.device), fake_labels.to(self.G.device))
        ...
        d_pred_fake = self.D(fake_data.to(self.D.device).detach(), fake_labels.to(self.D.device))
        error_fake = self.criterion(
            d_pred_fake, zeros_target(N).to(self.D.device)
        )
